# LeetCode Solution: Relative Ranks

## Problem
📋 **506. Relative Ranks**  
Check out the problem description on LeetCode:
👉 [https://leetcode.com/problems/relative-ranks/description/](https://leetcode.com/problems/relative-ranks/description/)

## My Solution
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:
👉 [https://leetcode.com/problems/relative-ranks/solutions/7359586/very-easy-sol-with-logic-explaination-by-obzh/](https://leetcode.com/problems/relative-ranks/solutions/7359586/very-easy-sol-with-logic-explaination-by-obzh/)

## Quick Overview
- **Problem Type**: Array, Sorting
- **Difficulty**: Easy
- **Key Concept**: Sorting, HashMap
- **Solution**: Assign relative ranks to athletes based on their scores

**Visit the links above to see the full problem statement and my detailed solution!** 🚀

---

## 👨‍💻 Author

**Vishal Yadav**  

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)