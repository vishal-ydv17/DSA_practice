# LeetCode Solution: Height Checker

## Problem  
📋 **1051. Height Checker**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/height-checker/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/height-checker/solutions/7468346/very-simple-sol-with-logic-explaination-w2m0b/

## Quick Overview
- **Problem Type**: Array  
- **Difficulty**: Easy  
- **Key Concept**: Compare original with sorted version  
- **Solution Summary**:  
  Compare the given array with its sorted version and count how many indices differ.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)
