# LeetCode Solution: Smallest Range I

## Problem  
📋 **908. Smallest Range I**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/smallest-range-i/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/smallest-range-i/solutions/7432905/very-easy-sol-with-logic-explaination-by-oni0/

## Quick Overview
- **Problem Type**: Array, Greedy  
- **Difficulty**: Easy  
- **Key Concept**: Min–Max difference reduction  
- **Solution Summary**:  
  Find the minimum and maximum values in the array.  
  Each element can be increased or decreased by at most `K`.  
  The smallest possible range is:  
  `max(0, maxValue - minValue - 2 * K)`.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)
