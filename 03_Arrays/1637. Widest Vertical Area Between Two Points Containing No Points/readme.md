# LeetCode Solution: Widest Vertical Area Between Two Points Containing No Points

## Problem  
📋 **1637. Widest Vertical Area Between Two Points Containing No Points**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/widest-vertical-area-between-two-points-containing-no-points/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/widest-vertical-area-between-two-points-containing-no-points/solutions/7520110/very-simple-sol-with-logic-explaination-h0n9u/

## Quick Overview
- **Problem Type**: Array, Sorting  
- **Difficulty**: Easy  
- **Key Concept**: Maximum gap between sorted x-coordinates  
- **Solution Summary**:  
  Extract all **x-coordinates** from points.  
  Sort them in ascending order.  
  Compute the **maximum difference** between consecutive x values.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal-ydv17)
