# LeetCode Solution: Sort Array by Increasing Frequency

## Problem  
📋 **1636. Sort Array by Increasing Frequency**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/sort-array-by-increasing-frequency/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/sort-array-by-increasing-frequency/solutions/7517732/very-simple-sol-with-logic-explaination-fqxz2/

## Quick Overview
- **Problem Type**: Array, Sorting, Hash Map  
- **Difficulty**: Easy  
- **Key Concept**: Custom sort with frequency  
- **Solution Summary**:  
  Count the frequency of each number.  
  Sort the array so that numbers with **lower frequency come first**.  
  If two numbers have the same frequency, the **larger number comes first**.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal-ydv17)
