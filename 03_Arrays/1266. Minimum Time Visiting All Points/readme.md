```md
# LeetCode Solution: Minimum Time Visiting All Points

## Problem  
📋 **1266. Minimum Time Visiting All Points**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/minimum-time-visiting-all-points/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/minimum-time-visiting-all-points/solutions/7480384/very-simple-sol-with-logic-explaination-tzedk/

## Quick Overview
- **Problem Type**: Array, Math  
- **Difficulty**: Easy  
- **Key Concept**: Manhattan distance  
- **Solution Summary**:  
  The time to move between two points is the **maximum of the x-distance and y-distance** because you can move diagonally.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal-ydv17)
```
