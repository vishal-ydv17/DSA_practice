```md
# LeetCode Solution: Kids With the Greatest Number of Candies

## Problem  
📋 **1431. Kids With the Greatest Number of Candies**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/kids-with-the-greatest-number-of-candies/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/kids-with-the-greatest-number-of-candies/solutions/7492687/very-simple-sol-with-logic-explaination-a0mx1/

## Quick Overview
- **Problem Type**: Array  
- **Difficulty**: Easy  
- **Key Concept**: Compare with max + extra candies  
- **Solution Summary**:  
  Find the **maximum candies** any kid currently has.  
  For each kid, check if adding `extraCandies` would make their total ≥ that maximum and collect results.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal-ydv17)
```
