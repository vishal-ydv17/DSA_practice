```md
# LeetCode Solution: Squares of a Sorted Array

## Problem  
📋 **977. Squares of a Sorted Array**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/squares-of-a-sorted-array/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/squares-of-a-sorted-array/solutions/7453988/very-simple-sol-with-logic-explaination-ujqe8/

## Quick Overview
- **Problem Type**: Array, Two Pointers  
- **Difficulty**: Easy  
- **Key Concept**: Two-pointer fill from ends  
- **Solution Summary**:  
  Use two pointers at the beginning and end of the sorted array.  
  Compare the absolute values, square the larger, and fill the result array from the end to front.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)
```
