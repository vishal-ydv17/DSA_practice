# LeetCode Solution: Can Make Arithmetic Progression From Sequence

## Problem  
📋 **1502. Can Make Arithmetic Progression From Sequence**  
Check out the problem description on LeetCode:  
👉 https://leetcode.com/problems/can-make-arithmetic-progression-from-sequence/description/

## My Solution  
💡 **Detailed explanation with examples**  
Read my complete solution on LeetCode:  
👉 https://leetcode.com/problems/can-make-arithmetic-progression-from-sequence/solutions/7498763/very-simple-sol-with-logic-explaination-jot1z/

## Quick Overview
- **Problem Type**: Array, Sorting  
- **Difficulty**: Easy  
- **Key Concept**: Arithmetic progression after sorting  
- **Solution Summary**:  
  Sort the array.  
  Check if the difference between every pair of consecutive elements is the same.

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)  
> 💼 **Portfolio:** [GitHub Profile](https://github.com/vishal-ydv17)  
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)  
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal-ydv17)
