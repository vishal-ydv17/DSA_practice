# LeetCode Solution: Middle of the Linked List

## Problem

📋 **876. Middle of the Linked List**
Check out the problem description on LeetCode:
👉 [https://leetcode.com/problems/middle-of-the-linked-list/](https://leetcode.com/problems/middle-of-the-linked-list/)

## My Solution

💡 **Detailed explanation with examples**
Read my complete solution on LeetCode:
👉 [https://leetcode.com/problems/middle-of-the-linked-list/solutions/8437202/very-easy-sol-with-logic-explaination-in-5i0e/](https://leetcode.com/problems/middle-of-the-linked-list/solutions/8437202/very-easy-sol-with-logic-explaination-in-5i0e/)

## Quick Overview

* **Problem Type**: Linked List, Two Pointers
* **Difficulty**: Easy
* **Key Concept**: Slow and Fast Pointers
* **Solution**: Use two pointers where the slow pointer moves one step at a time and the fast pointer moves two steps. When the fast pointer reaches the end of the list, the slow pointer will be at the middle node.

**Visit the links above to see the full problem statement and my detailed solution!** 🚀

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)
> 💼 **GitHub:** [GitHub Profile](https://github.com/vishal-ydv17)
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)
