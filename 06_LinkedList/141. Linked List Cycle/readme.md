<<<<<<< HEAD
# LeetCode Solution: Linked List Cycle

## Problem

📋 **141. Linked List Cycle**
Check out the problem description on LeetCode:
👉 [https://leetcode.com/problems/linked-list-cycle/description/](https://leetcode.com/problems/linked-list-cycle/description/)

## My Solution

💡 **Detailed explanation with examples**
Read my complete solution on LeetCode:
👉 [https://leetcode.com/problems/linked-list-cycle/solutions/8437275/easy-sol-with-logic-explaination-in-simp-7za5/](https://leetcode.com/problems/linked-list-cycle/solutions/8437275/easy-sol-with-logic-explaination-in-simp-7za5/)

## Quick Overview

* **Problem Type**: Linked List, Two Pointers
* **Difficulty**: Easy
* **Key Concept**: Floyd's Cycle Detection Algorithm (Tortoise and Hare)
* **Solution**: Use two pointers where the slow pointer moves one step at a time and the fast pointer moves two steps. If the linked list contains a cycle, the two pointers will eventually meet; otherwise, the fast pointer will reach the end of the list.

**Visit the links above to see the full problem statement and my detailed solution!** 🚀

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)
> 💼 **GitHub:** [GitHub Profile](https://github.com/vishal-ydv17)
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)
=======
# LeetCode Solution: Linked List Cycle

## Problem

📋 **141. Linked List Cycle**
Check out the problem description on LeetCode:
👉 [https://leetcode.com/problems/linked-list-cycle/description/](https://leetcode.com/problems/linked-list-cycle/description/)

## My Solution

💡 **Detailed explanation with examples**
Read my complete solution on LeetCode:
👉 [https://leetcode.com/problems/linked-list-cycle/solutions/8437275/easy-sol-with-logic-explaination-in-simp-7za5/](https://leetcode.com/problems/linked-list-cycle/solutions/8437275/easy-sol-with-logic-explaination-in-simp-7za5/)

## Quick Overview

* **Problem Type**: Linked List, Two Pointers
* **Difficulty**: Easy
* **Key Concept**: Floyd's Cycle Detection Algorithm (Tortoise and Hare)
* **Solution**: Use two pointers where the slow pointer moves one step at a time and the fast pointer moves two steps. If the linked list contains a cycle, the two pointers will eventually meet; otherwise, the fast pointer will reach the end of the list.

**Visit the links above to see the full problem statement and my detailed solution!** 🚀

---

## 👨‍💻 Author

**Vishal Yadav**

> 📧 **Email:** [vishalyadavy0022@gmail.com](mailto:vishalyadavy0022@gmail.com)
> 💼 **GitHub:** [GitHub Profile](https://github.com/vishal-ydv17)
> 🔗 **LinkedIn:** [linkedin.com/in/vishal-ydv17](https://linkedin.com/in/vishal-ydv17)
> 🐦 **Twitter:** [x.com/vishal_ydv17](https://x.com/vishal_ydv17)
>>>>>>> aed3457f1c2fda034e6693f08bdeb481ed2f0bcc
